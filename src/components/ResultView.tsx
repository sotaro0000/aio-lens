...
const IMPACT_META: Record<string, { label: string; cls: string }> = {
  ...
  low: { label: "効果 小", cls: "text-slate-500" },
};
      ...
      <section className="bg-white p-6 sm:p-8">
        <div className="flex flex-col items-center gap-8 sm:flex-row">
          ...
          <div className="min-w-0 flex-1">
            ...
            <h2 className="mt-1 break-words font-display text-lg font-semibold text-slate-900">
            ...
            <a
              ...
              className="break-all font-mono text-xs text-slate-400 underline-offset-2 hover:text-slate-700 hover:underline"
            >
              {result.finalUrl}
            </a>
            <dl className="mt-4 grid grid-cols-2 gap-x-6 gap-y-2 sm:grid-cols-4">
              {[
                ...
                <div key={x.k}>
                  ...
                  <dd className="font-mono text-sm text-slate-800">{x.v}</dd>
                </div>
              ))}
            </dl>
          </div>
        ...
        <section className="bg-white p-6 sm:p-8">
            ...
            <ol className="mt-5 divide-y divide-slate-100 border-t border-slate-100">
              {llm.priorityActions.map((a, i) => (
                <li key={i} className="flex gap-4 py-3">
                  <span className="font-mono text-sm text-slate-300">{String(i + 1).padStart(2, "0")}</span>
          ...
          {llm.generatedJsonLd && (
            <details className="mt-5 border border-slate-200">
              <summary className="cursor-pointer bg-slate-50 px-3 py-2 font-mono text-xs text-slate-600">
                推奨 JSON-LD サンプルを表示
              </summary>
              <pre className="overflow-x-auto bg-ink p-4 text-xs leading-relaxed text-slate-100">
